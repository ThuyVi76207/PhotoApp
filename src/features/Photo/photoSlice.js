...
const photo = createSlice({
    ...
    reducers: { //Tao action
        ...
        updatePhoto: (state, action) => {
            ...
            const photoIndex = state.findIndex(photo => photo.id === newPhoto.id); //tra ve number

            if (photoIndex >= 0) {
                state[photoIndex] = newPhoto; //cap nhat truc tiep state
            }
        }
    }
})
...
export const { addPhoto, removePhoto, updatePhoto } = actions; //export all actions
